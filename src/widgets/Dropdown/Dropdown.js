...
const Dropdown = ({ dropdownname, results, dropdownsearch = true, onChange, value, name }) => {
  ...
  const handleOptionClick = (option) => {
    ...
    setIsOpen(false);
    onChange({ target: { name, value: option } }); // Formik-compatible event
  };
  ...
  // Conditionally render the search box if there are more than 10 results
  const showSearchBox = filteredResults.length > 1;
